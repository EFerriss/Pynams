...
class Spectrum():
    ...
    The filetype defaults to .CSV, and that's the filetype my code handles
    best, although I've done ok with .txt. If in the future you want to handle
    other file types, that should be a relatively straightforward edit here.
    
    The sample should be a pynams Sample object, and any thickness information
    should be stored either with that Sample or it can be directly input as
    thickness_microns here. The raypath and IR polarization information can
    be included here. 
...
#                 peak_diffusivities=[], peak_diff_error=[], thickness_microns=None):
...
#        for spec in self.spectra_list:
#           if len(self.thickness_microns_list) < len(self.spectra_list):
#                if self.thickness_microns is not None:      
